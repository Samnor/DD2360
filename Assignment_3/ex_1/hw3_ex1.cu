...
__device__ float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    ...
    //////////////////////////////////////////////////////////////
    // It makes sense because we do not want to transfer data between host and device.
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out) {
    ...
    if (index_x < (width - 2) && index_y < (height - 2)) {
        ...
        __syncthreads();
        float gx = gpu_applyFilter(&sh_block[shared_id], BLOCK_SIZE_SH, sobel_x, 3);
        float gy = gpu_applyFilter(&sh_block[shared_id], BLOCK_SIZE_SH, sobel_y, 3);
        image_out[index_y * width + (index_x + 1)] = sqrtf(gx * gx + gy * gy);
    }
}
